    ...
    if raw_data:
        clean_data = [
            {
                ...
                'rank': crypto.get('market_cap_rank', 5000),
                'change_1h': float(
                    crypto.get('price_change_percentage_1h_in_currency', 0) / 100
                )
                if crypto.get('price_change_percentage_1h_in_currency') is not None
                else 0,
                'change_24h': float(
                    crypto.get('price_change_percentage_24h_in_currency', 0) / 100
                )
                if crypto.get('price_change_percentage_24h_in_currency') is not None
                else 0,
                'change_7d': float(
                    crypto.get('price_change_percentage_7d_in_currency', 0) / 100
                )
                if crypto.get('price_change_percentage_7d_in_currency') is not None
                else 0,
                'change_30d': float(
                    crypto.get('price_change_percentage_30d_in_currency', 0) / 100
                )
                if crypto.get('price_change_percentage_30d_in_currency') is not None
                else 0,
                'change_1y': float(
                    crypto.get('price_change_percentage_1y_in_currency', 0) / 100
                )
                if crypto.get('price_change_percentage_1y_in_currency') is not None
                else 0,
            }
            for crypto in raw_data
        ]
        db_token_list = [Token.model_validate(tok) for tok in clean_data]
        ...
        async for session in get_session():
            ...
            statement = select(Token).where(
                Token.updated_at < datetime.now() - timedelta(hours=2)
            )
...
async def get_next_execution_time():
    ...
    # Calculer combien de minutes on doit ajouter pour atteindre la prochaine minute paire
    minutes_to_add = ((now.minute + 1) % 2) + 1

    # Calcul de la prochaine exécution à 00 secondes
    next_execution_time = now.replace(second=0, microsecond=0) + timedelta(
        minutes=minutes_to_add
    )

    # next_execution_time = now + timedelta(seconds=5)
...
async def task_runner():
    ...
    while True:
        ...
        # print(
        #     f"Attente jusqu'à {next_execution_time} ({time_to_wait} secondes restantes)."
        # )
