...
async def get_cash_in_usd(transactions: list[dict], df_pf_history):
    ...
    # Il faut gérer si il y en a plusieurs à la meme date...
    ...
    for tr in transactions:
        ...
        if trx.type == 'Vente':
            ...
            # cash_in = cash_in * (1 - (trx_value / pf_value_end_of_day))
            # cash_in_array.append({'date': trx.date, 'cash_in_fiat_usd': cash_in})
        ...
        if t.actif_f_id != t.actif_a_id and t.actif_f_id is not None:
            try:
                if t.actif_f_id == fiat:
                    ...
                    function_fiat_price = await get_fiat_price(fiat, date_normalized, session)
                    val += t.qty_f / t.price * t.value_a / function_fiat_price
                    ...
                    function_fiat_price = await get_fiat_price(fiat, date_normalized, session)
                    val += t.qty_f * t.value_f / function_fiat_price
