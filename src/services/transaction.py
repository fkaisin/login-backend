...
class TransactionService:
    ...
    async def get_user_transactions(self, current_user_uid):
        ...
        statement = (
            ...
            .options(
                selectinload(User.transactions).selectinload(Transaction.actif_a).load_only(Token.symbol, Token.rank, Token.name), 
                selectinload(User.transactions).selectinload(Transaction.actif_v).load_only(Token.symbol, Token.rank, Token.name), 
                selectinload(User.transactions).selectinload(Transaction.actif_f).load_only(Token.symbol, Token.rank, Token.name), 
            )
        )  # fmt: skip
    ...
    async def delete_transaction(self, trx_id, current_user_uid):
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail='Transaction not found in DB.',
            ) from err
        except Exception as err:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f'Error retrieving transaction: {str(err)}',
            ) from err
