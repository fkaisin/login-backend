...
from src.db.models import DtaoCgList, Token
...
#                         'price': clean_price(subnet.price) * tao_price,
#                         'mcap': int(
#                             (clean_price(subnet.alpha_in) + clean_price(subnet.alpha_out))
#                             * clean_price(subnet.price)
#                             * tao_price
#                         ),
...
async def fetch_cg_ids_on_coingecko_async_task():
        ...
        async with session.get(COINGECKO_MARKET_URL, params=params) as resp:
            ...
            for coin in data:
                ...
                if symbol.startswith('SN'):
                    # cg_entries.append((coin['id'], symbol))  # stocker sous forme de tuple
    ...
    async for session in get_session():
        try:
            ...
            # Récupération des cg_id existants
            # existing_ids = set(await session.exec(select(DtaoCgList.cg_id)))

            # Filtrage des nouveaux
            # new_entries = [
            #     DtaoCgList(cg_id=cg_id, symbol=symbol) for cg_id, symbol in cg_entries if cg_id not in existing_ids
            # ]

            # if new_entries:
            #     session.add_all(new_entries)
            #     await session.commit()
            #     print(f'{len(new_entries)} nouveaux cg_id insérés.')
            # else:
            #     print('Aucune nouvelle entrée à insérer.')
